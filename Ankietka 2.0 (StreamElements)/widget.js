...
window.addEventListener('onEventReceived', function (obj) {
        ...
        if (((userstate.mod && userOptions['managePermissions'] === 'mods') || userstate.badges.broadcaster) && message.startsWith(userOptions['startCommand'])) {
            if (message.startsWith(userOptions['startCommand']) && !isActive) {
                ...
                users = [];
                temat = message.replace(userOptions['startCommand'],"");
                document.getElementById("temacik").innerHTML= temat;
                votes = {a: 0, b: 0};
                isActive = true;
...
////  document.getElementById("optiona").style= `heigth: ${sizea}%`;
    $("#optionb").css("height", `${sizeb}%`);
 //// document.getElementById("optionb").style= `heigth: ${sizeb}%`;
}
